...
public class BinaryLastOccurrence {
    public static void main(String[] args) {
        ...
        int key = sc.nextInt();
        int ans = LastOccurrences(arr, key, 0,arr.length-1);
        System.out.println("the last occurrences is = " +ans);

    }
    public static int LastOccurrences(int []arr , int key , int start , int end)
    {
        int result = -1;
        while (start <= end ) // its execute till that start is smaller and not equal end
        {
            ...
            if(arr[mid] == key) {
                ...
                start = mid+1; // for continue searching element for first occurrence
